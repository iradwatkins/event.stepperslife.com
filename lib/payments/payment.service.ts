import { paymentsApi, ordersApi, customersApi, SQUARE_CONFIG } from './square.config';
...
export class PaymentService {
  ...
  async createCustomer(params: {
    ...
    try {
      ...
      const { result } = await customersApi.createCustomer(request);
  ...
  async refundPayment(paymentId: string, reason?: string, amountToRefund?: number) {
    try {
      ...
      const { result } = await paymentsApi.refundPayment(refundRequest);
