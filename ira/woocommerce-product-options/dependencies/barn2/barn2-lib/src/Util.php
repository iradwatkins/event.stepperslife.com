...
class Util
{
    ...
    {
        if (empty($link_text)) {
            $link_text = __('Read more', 'woocommerce-product-options');
        }
    ...
    public static function install_bonus_plugins($bonus_plugins)
    {
        ...
        foreach ($bonus_plugins as $plugin) {
            ...
            if (is_wp_error($result)) {
                ...
                $error_message = __('Unable to connect to the filesystem. Please confirm your credentials.', 'woocommerce-product-options');
            ...
            if ($plugin && current_user_can('activate_plugin', $plugin)) {
                ...
                $results[$name] = new WP_Error('bonus_download_no_activation_permission', esc_html__('You don\'t have permission to activate the plugin.', 'woocommerce-product-options'));
                continue;
            }
            $results[$name] = \true;
        }
