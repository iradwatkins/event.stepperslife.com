...
namespace Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\Eloquent\Concerns;

use Barn2\Plugin\WC_Product_Options\Dependencies\Carbon\CarbonImmutable;
...
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Contracts\Database\Eloquent\CastsInboundAttributes;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Contracts\Support\Arrayable;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\Eloquent\Casts\AsArrayObject;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\Eloquent\Casts\AsCollection;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\Eloquent\Casts\Attribute;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\Eloquent\InvalidCastException;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\Eloquent\JsonEncodingException;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\Eloquent\Relations\Relation;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\LazyLoadingViolationException;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Support\Arr;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Support\Carbon;
...
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Support\Facades\Date;
...
trait HasAttributes
{
    ...
    protected function getRelationshipFromMethod($method)
    {
        ...
        return \Barn2\Plugin\WC_Product_Options\Helpers::tap($relation->getResults(), function ($results) use ($method) {
    ...
    {
        return \Barn2\Plugin\WC_Product_Options\Helpers::tap($this->getArrayAttributeByKey($key), function (&$array) use ($path, $value) {
    ...
    protected function getOriginalWithoutRewindingModel($key = null, $default = null)
    {
        ...
        return \Barn2\Plugin\WC_Product_Options\Helpers::collect($this->original)->mapWithKeys(function ($value, $key) {
    ...
    {
        static::$getAttributeMutatorCache[$class] = \Barn2\Plugin\WC_Product_Options\Helpers::collect($attributeMutatorMethods = static::getAttributeMarkedMutatorMethods($class))->mapWithKeys(function ($match) {
            return [lcfirst(static::$snakeAttributes ? Str::snake($match) : $match) => \true];
        })->all();
        static::$mutatorCache[$class] = \Barn2\Plugin\WC_Product_Options\Helpers::collect(static::getMutatorMethods($class))->merge($attributeMutatorMethods)->map(function ($match) {
    ...
    protected static function getAttributeMarkedMutatorMethods($class)
    {
        ...
        return \Barn2\Plugin\WC_Product_Options\Helpers::collect((new ReflectionClass($instance))->getMethods())->filter(function ($method) use ($instance) {
