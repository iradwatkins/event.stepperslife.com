...
namespace Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\Query;

use BackedEnum;
use Closure;
use DateTimeInterface;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Contracts\Support\Arrayable;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\Concerns\BuildsQueries;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\Concerns\ExplainsQueries;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\ConnectionInterface;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\Eloquent\Builder as EloquentBuilder;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\Eloquent\Relations\Relation;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\Query\Grammars\Grammar;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Database\Query\Processors\Processor;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Pagination\Paginator;
...
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Support\LazyCollection;
...
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Support\Traits\Macroable;
...
class Builder
{
    ...
    {
        return \Barn2\Plugin\WC_Product_Options\Helpers::collect($this->onceWithColumns(Arr::wrap($columns), function () {
    ...
    public function paginate($perPage = 15, $columns = ['*'], $pageName = 'page', $page = null)
    {
        ...
        $results = $total ? $this->forPage($page, $perPage)->get($columns) : \Barn2\Plugin\WC_Product_Options\Helpers::collect();
    ...
    {
        $this->enforceOrderBy();
        return \Barn2\Plugin\WC_Product_Options\Helpers::collect($this->orders ?? $this->unionOrders ?? [])->filter(function ($order) {
    ...
    public function pluck($column, $key = null)
    {
        ...
        if (empty($queryResult)) {
            return \Barn2\Plugin\WC_Product_Options\Helpers::collect();
        }
    ...
    protected function stripTableForPluck($column)
    {
        ...
        return \Barn2\Plugin\WC_Product_Options\Helpers::last(preg_split('~' . $separator . '~i', $column));
    }
    /**
    ...
    protected function pluckFromObjectColumn($queryResult, $column, $key)
    {
        ...
        return \Barn2\Plugin\WC_Product_Options\Helpers::collect($results);
    }
    /**
    ...
    public function upsert(array $values, $uniqueBy, $update = null)
    {
        ...
        $this->applyBeforeQueryCallbacks();
        $bindings = $this->cleanBindings(array_merge(Arr::flatten($values, 1), \Barn2\Plugin\WC_Product_Options\Helpers::collect($update)->reject(function ($value, $key) {
            return is_int($key);
        })->all()));
    ...
    {
        return \Barn2\Plugin\WC_Product_Options\Helpers::collect($bindings)->reject(function ($binding) {
    ...
    {
        return is_array($value) ? \Barn2\Plugin\WC_Product_Options\Helpers::head(Arr::flatten($value)) : $value;
    }
    /**
    ...
    {
        return \Barn2\Plugin\WC_Product_Options\Helpers::tap($this->clone(), function ($clone) use ($properties) {
    ...
    {
        return \Barn2\Plugin\WC_Product_Options\Helpers::tap($this->clone(), function ($clone) use ($except) {
