...
class SqlServerGrammar extends Grammar
{
    ...
    {
        $alias = \Barn2\Plugin\WC_Product_Options\Helpers::last(explode(' as ', $table));
    ...
    public function compileUpsert(Builder $query, array $values, array $uniqueBy, array $update)
    {
        ...
        $parameters = \Barn2\Plugin\WC_Product_Options\Helpers::collect($values)->map(function ($record) {
            return '(' . $this->parameterize($record) . ')';
        })->implode(', ');
        $sql .= 'using (values ' . $parameters . ') ' . $this->wrapTable('laravel_source') . ' (' . $columns . ') ';
        $on = \Barn2\Plugin\WC_Product_Options\Helpers::collect($uniqueBy)->map(function ($column) use ($query) {
            return $this->wrap('laravel_source.' . $column) . ' = ' . $this->wrap($query->from . '.' . $column);
        })->implode(' and ');
        $sql .= 'on ' . $on . ' ';
        if ($update) {
            $update = \Barn2\Plugin\WC_Product_Options\Helpers::collect($update)->map(function ($value, $key) {
