...
use Closure;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Support\Traits\Conditionable;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Support\Traits\Macroable;
use Barn2\Plugin\WC_Product_Options\Dependencies\Illuminate\Support\Traits\Tappable;
...
class Stringable implements JsonSerializable
{
    ...
    {
        return new static(\Barn2\Plugin\WC_Product_Options\Helpers::class_basename($this->value));
    }
    /**
    ...
    {
        return \Barn2\Plugin\WC_Product_Options\Helpers::collect(explode($delimiter, $this->value, $limit));
    }
    /**
    ...
    public function split($pattern, $limit = -1, $flags = 0)
    {
        if (filter_var($pattern, \FILTER_VALIDATE_INT) !== \false) {
            return \Barn2\Plugin\WC_Product_Options\Helpers::collect(mb_str_split($this->value, $pattern));
        }
        $segments = preg_split($pattern, $this->value, $limit, $flags);
        return !empty($segments) ? \Barn2\Plugin\WC_Product_Options\Helpers::collect($segments) : \Barn2\Plugin\WC_Product_Options\Helpers::collect();
    }
    /**
    ...
    {
        return \Barn2\Plugin\WC_Product_Options\Helpers::collect(sscanf($this->value, $format));
    }
    /**
    ...
    {
        return \Barn2\Plugin\WC_Product_Options\Helpers::collect(Str::ucsplit($this->value));
    }
    /**
